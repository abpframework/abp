...
- `apps/react/` — React SPA (main user-facing application)
- `apps/react-admin-console/` — React Admin Console (administration interface)
- `apps/auth-server/` — OpenIddict authentication server
- `gateways/web/` — YARP reverse proxy for the React apps
- `gateways/mobile/` — Gateway for mobile apps (only when `--mobile` is set)
