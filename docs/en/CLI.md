...
* **`switch-to-preview`**: Switches to the latest preview version of the ABP related packages on a solution.
...
* `--preview`: Use latest preview version (Only if `--version` is not specified and there is at least one pre-release after latest stable version).
