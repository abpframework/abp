...
Setting Management Module implements the `ISettingStore` (see [the setting system](../framework/fundamentals/settings)) to store the setting values in a database and provides the `ISettingManager` to manage (change) the setting values in the database.
...
Setting Management module is extensible, just like the [setting system](../framework/fundamentals/settings).  You can extend it by defining setting management providers. There are 5 pre-built setting management providers registered it the following order:
...
* [Settings](../framework/fundamentals/settings)
