...
> **React UI:** ABP Suite also generates React CRUD pages for modern React UI solutions (`app/react`), in addition to the MVC, Blazor and Angular UI stacks. The generator is template-based and does not use AI. It covers the standard CRUD scenarios — navigation-property lookups, master-detail child grids, file upload, enums, filtering, Excel export and bulk delete — and registers the page in the React app's route configuration and menu. Many-to-many navigation collections are not generated for React yet; for those, ABP license holders can use the [ABP Studio AI Agent](https://abp.io/studio/ai-agent) with predefined AI credits.
