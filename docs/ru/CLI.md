...
ABP CLI (Интерфейс командной строки) это инструмент командной строки для выполнения некоторых общих операций для решений на базе ABP.

## Установка

ABP CLI это [глобальный инструмент dotnet](https://docs.microsoft.com/en-us/dotnet/core/tools/global-tools). Установить его можно с помощью следующей команды в терминале:

````bash
dotnet tool install -g Volo.Abp.Cli
````

Для обновления уже установленного Abp Cli используйте команду:
...
Хоть каждая команда может иметь свой набор опций, есть несколько глобальных опций, которые можно использовать с любой командой;

* `--skip-cli-version-check`: Пропускает проверку обновлений ABP CLI. Если вы не укажете этот параметр, Abp Cli проверит наличие обновлений и покажет предупреждение, если существует более новая версия.

## Команды

Ниже приведен список всех доступных команд:

* **`help`**: Показывает справку по использованию ABP CLI.
* **`cli`**: Обновляет или удаляет ABP CLI.
* **`new`**: Генерирует новое решение на базе [стартовых шаблонов ABP](Startup-Templates/Index.md).
* **`update`**: Автоматическое обновление всех связанных с ABP пакетов NuGet и NPM в решении.
* **`clean`**: Удаляет все `BIN` и `OBJ` папки в текущей директории.
* **`add-package`**: Добавляет пакет ABP к проекту.
* **`add-module`**: Добавляет [многопакетный модуль приложения](https://docs.abp.io/en/abp/latest/Modules/Index) в решение.
* **`list-modules`**: Перечисляет имена модулей приложений с открытым исходным кодом.
* **`list-templates`**: Lists the names of available templates to create a solution.
* **`get-source`**: Скачивает исходный код модуля.
* **`generate-proxy`**: Генерирует клиентские прокси для использования эндпоинтов HTTP API.
* **`remove-proxy`**: Удаляет ранее созданные клиентские.
* **`switch-to-preview`**: Переключение на последнюю версию предварительного просмотра ABP Framework.
* **`switch-to-nightly`**: Переключает на последнюю версию [nightly сборок](Nightly-Builds.md) пакетов ABP, в решении.
* **`switch-to-stable`**: Переход на последние стабильные версии пакетов, входящих в ABP, в решении.
* **`translate`**: Упрощает перевод файлов локализации при наличии нескольких JSON [локализация](Localization.md) файлов в репозитарии.
* **`login`**: Аутентификация на вашем компьютере с помощью имени пользователя и пароля [abp.io](https://abp.io/).
* **`login-info`**: Показывает информацию о текущем пользователе.
* **`logout`**: Выход из системы с компьютера, если ранее вы прошли аутентификацию.
* **`bundle`**: Генерирует ссылки на скрипты и стили для проекта ABP Blazor и MAUI Blazor. 
* **`install-libs`**: Установка пакетов NPM для типов пользовательского интерфейса MVC / Razor Pages и Blazor Server.
