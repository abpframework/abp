...
我们将使用[ABP CLI](../../CLI.md) 创建带有ABP框架的新解决方案。 您可以在命令行终端中运行以下命令来安装它：
...
您可以使用此UI查看和测试您的HTTP API。现在，我们可以将 `TodoApp.Blazor` 设置为启动项目，并运行它来打开实际的Blazor应用程序UI:
...
您可以单击 *登录* 按钮，以`admin`作为用户名，`1q2w3E*` 作为密码登录到应用程序。
