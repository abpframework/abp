					...
					rowAction: {
						items: [
							...
							},
							{
								...
								visible: abp.auth.isGranted('Docs.Admin.Documents'),
								confirmMessage: function (data) {
									return l('ClearCacheConfirmationMessage', data.record.name);
								},
								action: function (data) {
							...
							},
							{
								...
								action: function (data) {
									volo.docs.admin.projectsAdmin
										...
										.then(function () {
											abp.message.success(
												l('SuccessfullyReIndexProject', data.record.name)
											);
										});
								},
							},
	...
	$('#ReIndexAllProjects').click(function (event) {
		abp.message
			...
			.done(function (accepted) {
				if (accepted) {
					volo.docs.admin.projectsAdmin.reindexAll().then(function () {
