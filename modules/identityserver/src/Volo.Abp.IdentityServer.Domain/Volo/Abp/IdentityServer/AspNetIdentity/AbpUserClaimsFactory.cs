...
namespace Volo.Abp.IdentityServer.AspNetIdentity
{
        ...
        where TUser : class
    {
        ...
        public AbpUserClaimsFactory(IObjectAccessor<IUserClaimsPrincipalFactory<TUser>> inner, UserManager<TUser> userManager)
        {
            _inner = inner;
        ...
        public async Task<ClaimsPrincipal> CreateAsync(TUser user)
        {
            ...
            if (_userManager.SupportsUserEmail)
            {
                ...
                if (!String.IsNullOrWhiteSpace(email))
                {
                    ...
                    identity.AddIfNotContains(new Claim(JwtClaimTypes.EmailVerified, await _userManager.IsEmailConfirmedAsync(user) ? "true" : "false", ClaimValueTypes.Boolean));
                }
            }
