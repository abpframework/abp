...
namespace Volo.Abp.TestApp.Testing
{
        ...
        where TStartupModule : IAbpModule
    {
        ...
        public async Task Should_Not_Allow_To_Update_If_The_Entity_Has_Changed()
        {
            ...
            var london1 = await CityRepository.FindByNameAsync("London").ConfigureAwait(false);
            ...
            await CityRepository.UpdateAsync(london2).ConfigureAwait(false);

            //And updating my old entity throws exception!
            await Assert.ThrowsAsync<AbpDbConcurrencyException>(async () =>
            {
                await CityRepository.UpdateAsync(london1).ConfigureAwait(false);
        ...
        public async Task Should_Not_Allow_To_Delete_If_The_Entity_Has_Changed()
        {
            ...
            await Assert.ThrowsAsync<AbpDbConcurrencyException>(async () =>
            {
                await CityRepository.DeleteAsync(london1).ConfigureAwait(false);
            }).ConfigureAwait(false);
        }
    }
}
