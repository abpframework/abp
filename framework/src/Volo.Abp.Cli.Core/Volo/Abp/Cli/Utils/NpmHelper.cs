...
public class NpmHelper : ITransientDependency
{
    ...
    public string GetYarnIgnoreScriptsOption(string directory)
    {
        // Yarn 2+ rejects --ignore-scripts, --mode=skip-build skips the install scripts there.
        // "--no" keeps npx from waiting on an install prompt the user can't see. If yarn isn't installed,
        // the later "npx yarn" call downloads the yarn package, which is Yarn 1.
        var version = FindVersionOrNull(CmdHelper.RunCmdAndGetOutput("npx --no -- yarn -v", directory));
        return version != null && version.Major >= 2 ? "--mode=skip-build" : "--ignore-scripts";
    }
    ...
    public void RunYarn(string directory)
    {
        ...
        CmdHelper.RunCmd($"npx yarn {GetYarnIgnoreScriptsOption(directory)}", directory);
    }
    ...
    public void YarnAddPackage(string package, string version, string directory)
    {
        ...
        EnsureSafeVersion(version);
        var packageVersion = !string.IsNullOrWhiteSpace(version) ? $"@{version}" : string.Empty;
        CmdHelper.RunCmd("npx yarn add " + package + packageVersion + " " + GetYarnIgnoreScriptsOption(directory), workingDirectory: directory);
    }
