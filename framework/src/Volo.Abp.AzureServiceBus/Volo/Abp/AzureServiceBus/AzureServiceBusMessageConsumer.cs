...
public class AzureServiceBusMessageConsumer : IAzureServiceBusMessageConsumer, ITransientDependency
{
        ...
        string connectionName)
    {
        ...
        Logger.LogDebug($"Initialize AzureServiceBusMessageConsumer Topic {topicName} subscription {subscriptionName}");
    ...
    protected virtual async Task HandleIncomingMessage(ProcessMessageEventArgs args)
    {
        try
        {
            ...
            stopWatch.Stop();
            Logger.LogDebug($"Processor {_topicName} message {args.Message.MessageId} {stopWatch.ElapsedMilliseconds} ms");
