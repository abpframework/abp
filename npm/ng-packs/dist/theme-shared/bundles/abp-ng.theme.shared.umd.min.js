!(function(t, e) {
...
})(this, function(t, e, n, o, a, r, i, s, p, l, c, u, d, m) {
  'use strict';
  ...
  var C = (function() {
    function t() {
      (this.buttonClass = 'btn btn-primary'), (this.loading = !1), (this.disabled = !1), (this.type = 'button');
    }
    return (
  ...
  var I = (function(t) {
    ...
    E = (p.Validators.minLength, p.Validators.required),
    k = ['newPassword', 'repeatNewPassword'],
    S = (function() {
      function t(t, e, o) {
        (this.fb = t),
          (this.store = e),
      ...
      return (
        ...
        (t.prototype.ngOnInit = function() {
          this.form = this.fb.group(
            { password: ['', E], newPassword: ['', E], repeatNewPassword: ['', E] },
            { validators: [l.comparePasswords(k)] },
          );
        }),
  ...
  var R = d.animation([d.style({ opacity: '0' }), d.animate('{{ time}} {{ easing }}', d.style({ opacity: '1' }))], {
      params: { time: '350ms', easing: 'ease' },
    }),
    N = d.animation([d.style({ opacity: '1' }), d.animate('{{ time}} {{ easing }}', d.style({ opacity: '0' }))], {
      params: { time: '350ms', easing: 'ease' },
    }),
    D = d.animation(
      [
        d.style({ opacity: '0', transform: '{{ transform }} translateY(-20px)' }),
        d.animate('{{ time }} {{ easing }}', d.style({ opacity: '1', transform: '{{ transform }} translateY(0)' })),
      ],
    ...
    M = d.animation(
      [
        d.style({ opacity: '0', transform: '{{ transform }} translateX(20px)' }),
        d.animate('{{ time }} {{ easing }}', d.style({ opacity: '1', transform: '{{ transform }} translateX(0)' })),
      ],
      { params: { time: '350ms', easing: 'ease', transform: '' } },
    ),
    _ = d.animation(
    ...
    F = d.animation(
      [
        d.style({ opacity: '1', transform: '{{ transform }} translateY(0)' }),
        d.animate('{{ time }} {{ easing }}', d.style({ opacity: '0', transform: '{{ transform }} translateY(20px)' })),
      ],
      { params: { time: '350ms', easing: 'ease', transform: '' } },
    ),
    B = d.animation(
    ...
    Y = d.animation(
      [
        d.style({ opacity: '1', transform: '{{ transform }} translateX(0)' }),
        d.animate('{{ time }} {{ easing }}', d.style({ opacity: '0', transform: '{{ transform }} translateX(20px)' })),
      ],
      { params: { time: '350ms', easing: 'ease', transform: '' } },
    ),
    q = d.animation(
    ...
    V = d.trigger('backdrop', [d.transition(':enter', d.useAnimation(R)), d.transition(':leave', d.useAnimation(N))]),
    $ = d.trigger('dialog', [d.transition(':enter', d.useAnimation(D)), d.transition(':leave', d.useAnimation(N))]),
    U = (function() {
      function t(t, e) {
        (this.renderer = t),
      ...
      function t(t, n, o, a, r, i, p, l, u) {
        ...
        (this.actions = t),
          ...
          t.pipe(s.ofActionSuccessful(e.RestOccurError)).subscribe(function(t) {
            ...
            if (n instanceof m.HttpErrorResponse && n.headers.get('_AbpErrorFormat')) {
              var a = d.showError(null, null, o);
              401 === n.status &&
              ...
              switch (n.status) {
                case 401:
                  d.showError(Q.defaultError401.details, Q.defaultError401.title).subscribe(function() {
                    return d.navigateToLogin();
                  });
                  break;
                case 403:
                  d.createErrorComponent({ title: Q.defaultError403.title, details: Q.defaultError403.details });
                  break;
                case 404:
                  d.showError(Q.defaultError404.details, Q.defaultError404.title);
                  break;
                case 500:
                  d.createErrorComponent({ title: Q.defaultError500.title, details: Q.defaultError500.details });
                  break;
                case 0:
                  'Unknown Error' === n.statusText &&
                    d.createErrorComponent({
                      title: Q.defaultErrorUnknown.title,
                      details: Q.defaultErrorUnknown.details,
                    });
                  break;
                default:
                  d.showError(Q.defaultError.details, Q.defaultError.title);
              }
          });
      }
      return (
        ...
        (t.prototype.createErrorComponent = function(t) {
          var e = this.rendererFactory.createRenderer(null, null),
            ...
            o = this.cfRes.resolveComponentFactory(z).create(this.injector);
  ...
  function nt(t) {
    return function() {
      ...
      return r.forkJoin(n.load(null, 'style', W, 'head', 'afterbegin')).toPromise();
    };
  }
  var ot,
    at = (function() {
            ...
            args: [
              {
                ...
                declarations: [w, C, S, O, P, z, A, U, Z, et, G, J],
                exports: [w, C, S, O, P, A, U, Z, et, G, J],
                entryComponents: [z],
              },
            ],
          },
    ...
    rt = d.animation(
      [
        d.style({ height: '*', overflow: 'hidden', 'box-sizing': 'border-box' }),
        d.animate('{{ time }} {{ easing }}', d.style({ height: '0', padding: '0px' })),
      ],
      { params: { time: '350ms', easing: 'ease' } },
    ),
    it = d.animation(
      [
        d.style({ width: '*', overflow: 'hidden', 'box-sizing': 'border-box' }),
        d.animate('{{ time }} {{ easing }}', d.style({ width: '0', padding: '0px' })),
      ],
      { params: { time: '350ms', easing: 'ease' } },
    ),
    st = d.animation(
      [
        d.style({ height: '0', overflow: 'hidden', 'box-sizing': 'border-box' }),
        d.animate('{{ time }} {{ easing }}', d.style({ height: '*', padding: '*' })),
      ],
      { params: { time: '350ms', easing: 'ease' } },
    ),
    pt = d.animation(
      [
        d.style({ width: '0', overflow: 'hidden', 'box-sizing': 'border-box' }),
        d.animate('{{ time }} {{ easing }}', d.style({ width: '*', padding: '*' })),
      ],
      { params: { time: '350ms', easing: 'ease' } },
    ),
    lt = d.trigger('collapse', [
      d.state('collapsed', d.style({ height: '0', overflow: 'hidden' })),
      d.state('expanded', d.style({ height: '*', overflow: 'hidden' })),
      d.transition('expanded => collapsed', d.useAnimation(rt)),
      d.transition('collapsed => expanded', d.useAnimation(st)),
    ]),
    ct = d.trigger('slideFromBottom', [
      d.transition('* <=> *', [
        d.style({ 'margin-top': '20px', opacity: '0' }),
        d.animate('0.2s ease-out', d.style({ opacity: '1', 'margin-top': '0px' })),
      ]),
    ]),
    ut = d.animation(
      [
        d.style({ opacity: '0', display: '{{ display }}' }),
        d.animate(
          '{{ time}} {{ easing }}',
          d.keyframes([
            d.style({ opacity: '0', transform: '{{ transform }} scale(0.0)', offset: 0 }),
            d.style({ opacity: '0', transform: '{{ transform }} scale(0.8)', offset: 0.5 }),
            d.style({ opacity: '1', transform: '{{ transform }} scale(1.0)', offset: 1 }),
          ]),
        ),
      ],
      {
  ...
  !(function(t) {
    ...
    (t.ButtonComponent = C),
    (t.ChangePasswordComponent = S),
    (t.ChartComponent = O),
    (t.ConfirmationComponent = P),
    (t.ConfirmationService = j),
    (t.LoaderBarComponent = A),
    (t.ModalComponent = U),
    (t.ProfileComponent = Z),
    (t.SortOrderIconComponent = J),
    (t.TableEmptyMessageComponent = et),
    (t.ThemeSharedModule = at),
    (t.ToastComponent = G),
    (t.ToasterService = I),
    (t.addSettingTab = function(t) {
      Array.isArray(t) || (t = [t]), mt.push.apply(mt, v(t));
    }),
    (t.appendScript = nt),
    (t.bounceIn = ut),
    (t.chartJsLoaded$ = L),
